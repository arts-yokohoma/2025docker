...
from app.repository.user_repository import AdminUserRepository
import random
from app.entity.user_admin_entity import (
...
def get_admin_user_repo(
    ...
    return AdminUserRepository(logger=logger, db=db)


@router.post("/create", response_model=UserAdminOut)
async def create_user_admin(
    user: UserAdminCreate,
    repo: AdminUserRepository = Depends(get_admin_user_repo),
    token_payload=Depends(get_auth_dependency([1, 2])),
):
    try:
        if (
        ...
        role_type = token_payload.get("role_type")
        if role_type == 2 and user.role_type == 1:
            return response(code=401, message="権限が無い")
...
@router.get("/", response_model=UserAdminOut)
async def list_users(
    ...
    try:
        ...
        users_list = [
            UserAdminOut.from_orm(user).model_dump(mode="json") for user in result
        ]
...
def login(user: LoginSchema, repo: AdminUserRepository = Depends(get_admin_user_repo)):
    try:
        if not user.username:
            return response(code=400, message="パスワードまたはユーザー名間違い")
        ...
        token_data = {"user_id": tmp_user.id, "role_type": tmp_user.role_type}
...
async def get_user_info(
    ...
    try:
        ...
        user_out = UserAdminOut.from_orm(user).model_dump(mode="json")
